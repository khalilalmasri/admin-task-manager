...
export default function CompanyImport() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      formData.append('companies', data.files);

      const response = await axios.post('/api/admin/import-company', formData, {
    ...
    <Form methods={methods} onSubmit={handleSubmit(onSubmit)}>
      <Card>
        <Stack spacing={3} sx={{ p: 3 }}>
          <Typography variant="h6">{t('import_company')}</Typography>
          <RHFUpload
            name="files"
            accept=".xlsx, .xls, .csv"
            maxSize={3145728}
