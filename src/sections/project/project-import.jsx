...
import ImportSuccessTableRowProjects from './import-success-projects';
...
export default function ProjectImport() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      formData.append('projects', data.files);

      const response = await axios.post('/api/admin/import-project', formData, {
    ...
    <Form methods={methods} onSubmit={handleSubmit(onSubmit)}>
      <Card>
        <Stack spacing={3} sx={{ p: 3 }}>
          <Typography variant="h6">{t('import_project')}</Typography>
          <RHFUpload
            name="files"
            accept=".xlsx, .xls, .csv"
            maxSize={3145728}
